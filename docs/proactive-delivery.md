...
Hermes cron jobs run in a [fresh isolated agent session](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/website/docs/user-guide/features/cron.md#L201-L259) by default. Hermes can opt a delivery into a continuable chat session on supported platforms, but broadcast delivery is deliberately not continuable. The current Responses and Sessions APIs expose request/run streams to their caller; the pinned API adapter has [no persistent outbound channel](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/gateway/platforms/api_server.py#L835-L843) and cannot push an unsolicited assistant turn into this Worker's idle per-device Responses chain.

The preferred Hermes-side integration is therefore a small `voice_pe` [platform-adapter plugin](https://hermes-agent.nousresearch.com/docs/developer-guide/adding-platform-adapters). Its `send()` implementation posts an authenticated delivery envelope to the Worker. It should register a cron home target and standalone sender so cron delivery also works outside a live gateway adapter. Cron auto-delivery, webhook cross-platform delivery, and the operator-facing `hermes send` command can then use the same adapter. An autonomous agent can schedule that delivery through Hermes' `cronjob` tool; immediate arbitrary outreach would require an explicit, separately permissioned `voice_delivery` plugin tool. Hermes' [generic send engine is intentionally not registered as a default model-callable tool](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/tools/send_message_tool.py#L1952-L1965), so the design must not assume the model can invoke `send_message`. A gateway hook that posts directly is acceptable for non-critical experiments, but hooks are non-blocking observers and should not be the durable delivery queue.
...
These interfaces were checked at the repository's pinned Hermes commit [`4aa499f`](https://github.com/NousResearch/hermes-agent/tree/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e) and current upstream [`7acaff5`](https://github.com/NousResearch/hermes-agent/tree/7acaff5ef2bcbaa22bd23b72efe60906123a4f55); the relevant source is unchanged. Both currently report package version 0.18.2, so the implementation must pin and re-audit an exact commit or container digest rather than relying on `>=0.18.2` alone.
...
Server-initiated playback changes turn ownership enough that it should use a negotiated protocol capability or a new `hermes-voice.realtime.v3` subprotocol, not an accidental extension of v2. A draft exchange is:
