...
The buffered diagnostic compatibility route remains:
...
It shares provider credentials and the stable long-term-memory scope, but uses its own `voice-buffered-<device-id>` named response chain. It neither reads nor advances realtime conversation state, does not share realtime performance/cancellation guarantees, and is deprecated for ordinary conversational use.
...
The Durable Object stores only the minimum continuity metadata. Audio, partial transcripts, Hermes deltas, tool payloads, and TTS bytes are transient. Its versioned record contains an opaque conversation UUID; the bound Hermes base URL, model/route label, and resolved session key; the last completed Hermes response ID; the `X-Hermes-Session-Id` returned with that response; last accepted-turn/completion activity time; and the last accepted reset request ID. A small current-turn journal records its conversation UUID, prior head, and in-flight response candidate. This survives device reconnects, ESP32 reboots, Worker deployment, and object hibernation without turning those transport events into conversation boundaries.
...
On reconnect, transport sequence state is new. A partially transmitted or ambiguous turn is not replayed automatically. The Durable Object keeps the local conversation UUID and last **completed** conversation head, so the next newly spoken turn can continue from a safe point. Reconnect, reboot, wake, cancellation, and barge-in are transport/turn events, not implicit new conversations.
...
4. Scribe may emit mutable `partial_transcript` events. Realtime v2 reserves an optional device status control for future diagnostics, but the shipped gateway discards provider partials to protect the bounded device control queue. They are never sent to Hermes.
...
6. The Durable Object starts Hermes `POST /v1/responses` with `stream: true`, `store: true`, the stable long-term-memory session key, and the last completed `previous_response_id` when one exists. The endpoint and API key select the Hermes profile; the request `model` value is cosmetic.
...
9. Each ElevenLabs audio message is base64-decoded and immediately emitted as a binary downlink PCM frame. The device acknowledges consumption so the gateway cannot build an unbounded playback backlog.
10. Hermes `response.completed` closes and flushes the TTS context. After final PCM has left the device receive ring and is cumulatively acknowledged, the gateway emits `turn.done`; the device remains in reply state until the downstream speaker path drains, then returns to wake-ready.
...
| Durable Object conversation UUID | One explicit short-term conversation incarnation. Rotates on an accepted reset, enabled idle expiry, a stale-head `404`, or a Hermes binding change. |
...
| `X-Hermes-Session-Key` | Operator-owned long-term-memory scope; stable across ordinary conversation UUID rotations, mapped by `HERMES_SESSION_KEYS_JSON` or defaulted to `voice:<device-id>`. |
...
The actual Hermes agent/profile is selected by `HERMES_BASE_URL` and `HERMES_API_KEY`, normally a profile-specific API server or route. `HERMES_MODEL` is an advertised model/route name accepted by the compatibility API, not a permission boundary. The Durable Object binds each conversation to the normalized base URL, that model/route label, and the resolved `X-Hermes-Session-Key`. If any changes, it rotates before the next accepted turn and omits the old `previous_response_id`; a response chain is never deliberately crossed into another agent route or memory scope.
...
A **reply** is the next non-empty committed Scribe transcript while the same Durable Object conversation UUID is current. It sends the last completed response ID, if any, and therefore includes prior user turns, assistant messages, function calls, and function results. A wake phrase, short button start, reconnect, device reboot, cancellation, or barge-in may create a new transport turn but remains a reply in the same conversation.

A **new conversation** creates a new Durable Object UUID and clears the completed response head and stored Hermes response-session metadata. Explicit, idle, and stale-`404` rotations leave `X-Hermes-Session-Key` unchanged; a binding-change rotation adopts the newly configured scope. It occurs only when:
...
2. the optional inactivity boundary is enabled and has elapsed before the next turn; or
3. Hermes returns `404` while the supplied completed head is present, which the gateway treats as that stored response no longer being available; or
4. the configured Hermes base URL, model/route label, or resolved long-term-memory session key differs from the binding persisted for that conversation.

The gateway never tries to infer a boundary from natural-language phrases such as “new topic.” Explicit user control is authoritative; inactivity rotation is off by default because no universal timeout matches every conversation.
...
When `CONVERSATION_IDLE_SECONDS` is set to 60–31,536,000, expiry is checked when a new turn is reserved, so no alarm or idle billing is required. Activity is updated atomically when a unique `turn.start` is accepted and refreshed when Hermes completes; a failed or cancelled accepted turn therefore still postpones idle expiry, and a long successful turn measures the next idle period from completion. `0`, `off`, `none`, or omission disables the policy.
...
binding                 Hermes base URL + model/route + resolved session key
...
5. After a Durable Object restart, `GET /v1/responses/{inflight_response_id}` may reconcile the journal. Only a retrieved response whose status is exactly `completed` can be promoted; `in_progress`, `incomplete`, `failed`, or missing responses are cleared as candidates without changing the safe head.
...
Hermes itself persists in-progress and incomplete responses for retrieval. Not promoting those IDs prevents a later turn from inheriting partial assistant text. Hermes named-conversation mappings advance as soon as the streaming response is created, which is why realtime v2 uses an explicit Durable-Object-owned pointer instead. Hermes cannot roll back tools already executed, so the gateway never automatically retries an ambiguous turn.

If Hermes returns `404` for the stored previous ID, it has rejected the request before running that turn. The gateway rotates the conversation UUID, clears the stale head/session metadata, fails the current spoken turn, and does **not** replay it. The next newly spoken turn starts the already-rotated fresh Hermes chain. This makes history loss visible for the affected turn without duplicating a command; the long-term-memory session key remains stable.
...
| One device PCM payload | 8 KiB; normal reframe target 2 KiB |
...
4. The Hermes SSE fetch is aborted. Hermes 0.18.2 attempts to interrupt its agent when the SSE client disconnects.
...
| --- | --- | --- |
| Voice PE → Worker | per-device bearer preferred | Compiled into firmware. Authenticates hardware, not the speaker. |
...
Every upstream base URL is HTTPS, redirects are rejected for credentialed requests, errors returned to the device exclude upstream bodies/transcripts, and CORS is disabled. Per-device tokens prevent a device from selecting another device's Durable Object or conversation head.
...
- The buffered `/v1/voice` route remains an operator-selected diagnostic, not an automatic replay target or conversational continuation path.
