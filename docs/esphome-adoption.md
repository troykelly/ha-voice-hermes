...
In Home Assistant, select the discovered **hermes-voice-pe-… – Improv via BLE** device (the suffix is derived from that unit's MAC), provide the 2.4 GHz Wi-Fi credentials, then press the Voice PE center button to authorize. A phone can perform the equivalent flow directly. Improv Serial over USB is the independent provisioning and recovery path.
...
Read **Hermes Device ID** in Home Assistant or the ESPHome logs. Add that ID and a fresh random token of at least 32 bytes to the Worker's `DEVICE_TOKENS_JSON`, and add an intentional long-term-memory scope for the same ID to `HERMES_SESSION_KEYS_JSON`. Deploy the Worker configuration before enrolling the device. In the YAML generated by Device Builder, add:
...
Add the three values to that Device Builder installation's `secrets.yaml`, validate, and install over OTA. Use a fresh high-entropy OTA password. The URL must be a credential-free `wss://.../v2/realtime` URL; the token must match the Worker's entry for the displayed hardware ID. Hermes, ElevenLabs, Access, profile, voice, and memory-scope credentials/settings remain in the Worker.
...
- Compile factory, imported-unconfigured and gateway-configured variants with the package minimum and pinned ESPHome 2026.6.5 validation patch.
