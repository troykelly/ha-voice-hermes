...
ESPHome's [`micro_wake_word`](https://esphome.io/components/micro_wake_word/) accepts a `models` list, individual model IDs, enable/disable actions, per-model sensitivity and VAD. Multiple-model support was introduced explicitly in [ESPHome 2024.7](https://esphome.io/changelog/2024.7.0/#microwakeword) and is present in the pinned ESPHome 2026.6.0 source.
...
- only the first configured wake model is enabled on a fresh device; external-model enablement is subsequently persisted, so listing Avery and Jordan is not enough to activate both ([codegen](https://github.com/esphome/esphome/blob/e2157a3d26a8959c7c7ff212ab40afdd7b9f9d13/esphome/components/micro_wake_word/__init__.py#L416-L531));
- every configured model must use the same manifest `feature_step_size`; mixed models are rejected at validation;
- enabled models infer sequentially over one shared feature stream rather than on parallel cores ([runtime loop](https://github.com/esphome/esphome/blob/e2157a3d26a8959c7c7ff212ab40afdd7b9f9d13/esphome/components/micro_wake_word/micro_wake_word.cpp#L429-L482));
- every enabled model has its own interpreter, tensor arena, variable arena and probability window; every model blob consumes firmware flash even while disabled ([model loader](https://github.com/esphome/esphome/blob/e2157a3d26a8959c7c7ff212ab40afdd7b9f9d13/esphome/components/micro_wake_word/streaming_model.cpp#L28-L105));
- `on_wake_word_detected` receives the manifest's human `wake_word` string, not its YAML model ID; manifests with the same label are indistinguishable through that callback ([model identity and persistence](https://github.com/esphome/esphome/blob/e2157a3d26a8959c7c7ff212ab40afdd7b9f9d13/esphome/components/micro_wake_word/streaming_model.cpp#L246-L307));
- two models can cross threshold for one utterance and queue two detections before `stop_after_detection` takes effect.

The [stock Voice PE configuration](https://github.com/esphome/home-assistant-voice-pe/blob/0579e7b9d8504264719c593474c85447253c9dc1/home-assistant-voice.yaml#L1716-L1775) demonstrates simultaneous primary/command-model behavior with a selected wake model, internal `Stop` model and VAD. Its Home Assistant [`voice_assistant` integration advertises only one selectable external wake word](https://github.com/esphome/esphome/blob/e2157a3d26a8959c7c7ff212ab40afdd7b9f9d13/esphome/components/voice_assistant/voice_assistant.cpp#L1085-L1135) at a time, but this project does not instantiate `voice_assistant`; local firmware can explicitly enable more than one model. That must be physically qualified because CPU time, probability-window latency and cross-model false accepts grow with the active set.
...
Hermes' Responses `model` field is [cosmetic by default](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/website/docs/user-guide/features/api-server.md#L493-L497). The optional [`platforms.api_server.extra.model_routes`](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/cli-config.yaml.example#L607-L640) feature can map an alias to a different inference provider/model/API key/base URL, but it remains inside the same profile: the SOUL, tools, sessions, built-in memory and response database do not change. Unknown aliases silently fall back to the profile default, and the alias is resolved independently on every request rather than inherited through `previous_response_id` ([route lookup](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/gateway/platforms/api_server.py#L1162-L1211)). Model routes are appropriate for “Avery fast” versus “Avery deep,” not Avery versus Jordan.

`X-Hermes-Session-Key` scopes provider-backed long-term memory such as Honcho, but it does not isolate profile-wide [`MEMORY.md` or `USER.md`](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/website/docs/user-guide/features/memory.md#L7-L20). Separate profiles are therefore required when Avery and Jordan must have different identity, memories or tool policy. Profiles are state boundaries, [not operating-system sandboxes](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/website/docs/user-guide/profiles.md#L125-L149): on the local terminal backend they may still share the host user's filesystem and CLI credentials unless separately containerized, sandboxed or configured with profile-specific home behavior.

Do not depend on a `/p/<profile>/v1/responses` multiplexer route. Current and pinned [API-server route registration](https://github.com/NousResearch/hermes-agent/blob/4aa499ff9f3fcc0c38ce61da46805a4dcc8f612e/gateway/platforms/api_server.py#L4769-L4809) exposes unprefixed `/v1/*` and `/api/*` routes. Use separate profile processes/ports, optionally placed behind explicit reverse-proxy or Cloudflare Tunnel hostnames.
...
Agent routing changes conversation ownership, so it belongs in the negotiated future `hermes-voice.realtime.v3` protocol rather than an undocumented v2 field.
...
- A route-specific stale Hermes `404` rotates/fails only that lane and never replays the utterance.
