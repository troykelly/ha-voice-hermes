...
The firmware and gateway compile; 84 Rust tests and 13 tests against the optimized, compiled Worker in workerd pass. Runtime coverage includes Durable Object hibernation/replacement, immediate durable-quota admission and side-effect gates, strict fail-closed provider redirects/status/media types, a hard decoded-output ceiling across irregular TTS chunks, and a byte-exact two-turn mocked Scribe → Hermes SSE → ElevenLabs TTS streaming conversation. No physical Voice PE or live Hermes/ElevenLabs credentials were available for the recorded run. Physical adoption/OTA, AEC/barge-in, impaired-Wi-Fi, playback, live-provider continuity, PII/DLP policy validation, and measured latency remain release gates; the repository deliberately has no release tag until they pass. See [`docs/testing.md`](docs/testing.md).
